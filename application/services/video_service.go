...
func (v *VideoService) Encode() error {
	...
	cmdArgs = append(cmdArgs, "--use-segment-timeline")                       // Faz picar em diversos arquivos, para não precisarmos carregar tudo de uma vez
	cmdArgs = append(cmdArgs, "--o")                                          // Falando que ele é output, onde que irá salvar
	cmdArgs = append(cmdArgs, os.Getenv("LOCAL_STORAGE_PATH")+"/"+v.Video.ID) // Pasta que irá salvar os arquivos
...
func (v *VideoService) Finish() error {
	...
	if err != nil {
		log.Println("error removing files in: ", v.Video.ID)
		return err
	}
